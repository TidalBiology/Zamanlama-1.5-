...
def update(dt):
    global death, mod, score, time_left, highscore

    # Ölüm veya süre bittiğinde oyunu bitir
    if mod == 1 and (time_left <= 0):
        mod = 2

        # Skoru kontrol et, yeni highscore belirle
        if score > highscore:
            highscore = score
...
def decrease_base_time():
    ...
    if base_time > min_time:
        base_time -= 1
        time_left = base_time  # Anlık süreyi de güncelle


def reset_game():
    """Oyunu sıfırla."""
    global score, death, base_time
    score = 0
    death = 0
